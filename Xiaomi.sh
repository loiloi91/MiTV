...
while true; do
    ...
    case $choice in
        ...
        2) echo -e "\n${B}[*] Đang tải file từ GitHub...${NC}"
           for f in "DLStore_v16.0.3.apk" "LeanKeyboard_6.1.28.apk" "ProjectivyLauncher.apk" "voice.apk"; do
           echo -ne "${Y}  > Tải $f... ${NC}"; curl -L "$GH/$f" -o "$DIR/$f" --silent && echo -e "${G}[OK]${NC}"; done
           read -p "Xong! Nhấn Enter...";;
        3) echo -e "\n${Y}[*] Đang cài bộ app lõi...${NC}"
           for f in "DLStore_v16.0.3.apk" "LeanKeyboard_6.1.28.apk" "ProjectivyLauncher.apk" "voice.apk"; do
           [ -f "$DIR/$f" ] && (echo -ne "${C}  > Cài $f... ${NC}"; adb install -r "$DIR/$f" >/dev/null 2>&1 && echo -e "${G}[XONG]${NC}"); done
           read -p "Xong! Nhấn Enter...";;
        ...
        5) echo -e "\n${R}[*] ĐANG GỠ APP RÁC...${NC}"
           ...
           for app in "${apps[@]}"; do echo -ne "${Y}  > Gỡ $app... ${NC}"; adb shell pm uninstall --user 0 $app >/dev/null 2>&1 && echo -e "${G}[SẠCH]${NC}"; done
           read -p "Xong! Nhấn Enter...";;
        [xX]) exit 0;;
        *) sleep 1;;
    esac
